...
func (h *AgentHandler) GenerateScript(c *gin.Context) {
	...
	shellScript += "# ProxyClaw Agent 一键配置脚本\n"
	shellScript += "# Agent: " + string(agentType) + "\n"
	shellScript += "# Route: " + routeName + "\n"
	shellScript += "# Generated by ProxyClaw\n\n"
	shellScript += "set -e\n\n"

	shellScript += "echo \"=== ProxyClaw Agent 配置 ===\"\n"
	...
	psScript := "# ProxyClaw Agent 一键配置脚本 (PowerShell)\n"
	psScript += "# Agent: " + string(agentType) + "\n"
	psScript += "# Route: " + routeName + "\n"
	psScript += "# Generated by ProxyClaw\n\n"
	psScript += "$ErrorActionPreference = 'Stop'\n\n"

	psScript += "Write-Host '=== ProxyClaw Agent 配置 ===' -ForegroundColor Cyan\n"
...
func (h *AgentHandler) respondProxyAPIKeyError(c *gin.Context, err error) {
	if errors.Is(err, errNoUsableProxyAPIKey) {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "未找到可用于 Agent 接入的 ProxyClaw API Key（llmproxy_*）。请先在“个人设置 -> API Keys”创建，并确保服务端配置了 LLM_PROXY_API_KEY_STORAGE_SECRET 以支持解密展示。",
		})
		return
	}
	c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
