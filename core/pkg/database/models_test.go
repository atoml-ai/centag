...
func TestUserRequestLog_Fields(t *testing.T) {
	now := time.Now()
	log := &UserRequestLog{
		ID:           1,
		UserID:       100,
	...
	if log.ID != 1 {
		t.Errorf("UserRequestLog.ID = %d, want 1", log.ID)
	}
	if log.UserID != 100 {
		t.Errorf("UserRequestLog.UserID = %d, want 100", log.UserID)
	}
	if log.TenantID != "tenant-001" {
		t.Errorf("UserRequestLog.TenantID = %s, want tenant-001", log.TenantID)
	}
	if log.RequestID != "req-abc123" {
		t.Errorf("UserRequestLog.RequestID = %s, want req-abc123", log.RequestID)
	}
	if log.Model != "gpt-4" {
		t.Errorf("UserRequestLog.Model = %s, want gpt-4", log.Model)
	}
	if log.Backend != "openai" {
		t.Errorf("UserRequestLog.Backend = %s, want openai", log.Backend)
	}
	if log.InputTokens != 500 {
		t.Errorf("UserRequestLog.InputTokens = %d, want 500", log.InputTokens)
	}
	if log.OutputTokens != 1000 {
		t.Errorf("UserRequestLog.OutputTokens = %d, want 1000", log.OutputTokens)
	}
	if log.LatencyMs != 250 {
		t.Errorf("UserRequestLog.LatencyMs = %d, want 250", log.LatencyMs)
	}
	if log.StatusCode != 200 {
		t.Errorf("UserRequestLog.StatusCode = %d, want 200", log.StatusCode)
	}
}

func TestUserRequestLog_DefaultFields(t *testing.T) {
	log := &UserRequestLog{
		ID:       1,
		UserID:   100,
	...
	// Default values
	if log.TenantID != "" {
		t.Errorf("default TenantID should be empty, got %s", log.TenantID)
	}
	if log.Pipeline != "" {
		t.Errorf("default Pipeline should be empty, got %s", log.Pipeline)
	}
	if log.InputTokens != 0 {
		t.Errorf("default InputTokens should be 0, got %d", log.InputTokens)
	}
	if log.OutputTokens != 0 {
		t.Errorf("default OutputTokens should be 0, got %d", log.OutputTokens)
	}
	if log.LatencyMs != 0 {
		t.Errorf("default LatencyMs should be 0, got %d", log.LatencyMs)
	}
	if log.StatusCode != 0 {
		t.Errorf("default StatusCode should be 0, got %d", log.StatusCode)
	}
}
