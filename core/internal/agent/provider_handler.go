...
// Create 创建配置
func (h *AgentProviderHandler) Create(c *gin.Context) {
	var cfg AgentProviderConfig
...
// Update 更新配置
func (h *AgentProviderHandler) Update(c *gin.Context) {
	id := c.Param("id")
	...
	cfg.ID = id
	if cfg.AgentType == "" {
		cfg.AgentType = existing.AgentType
	}
	if cfg.DisplayName == "" {
...
// Delete 删除配置
func (h *AgentProviderHandler) Delete(c *gin.Context) {
	id := c.Param("id")
