...
> **Логин по умолчанию:** логин `admin` / пароль `centag123`
> (рекомендуется сменить после первого входа; переопределить через `LLM_PROXY_ADMIN_PASSWORD` до первого запуска)
