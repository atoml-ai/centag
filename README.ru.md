...
> **Логин по умолчанию:** логин `admin` — пароль задаётся в мастере первого запуска (предустановленного пароля нет). Можно предустановить через `LLM_PROXY_ADMIN_PASSWORD` до первого запуска.
...
cd centag
mkdir -p ~/.centag && cp config/centag.conf.example ~/.centag/centag.conf   # при необходимости отредактируйте
./start.sh docker build personal                     # сборка образа
./start.sh docker up personal                        # запуск контейнера
```

Админка: http://localhost:20060 · Остановка: `./start.sh docker down`

Данные хранятся в `deploy/docker/data/` (создаётся автоматически при первом запуске).

<details>
<summary>Нативные Docker команды (альтернатива)</summary>
