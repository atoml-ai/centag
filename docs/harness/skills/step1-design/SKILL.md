---
name: step1-design
description: "工作流 Step 1：方案设计与确认 — 技术方案、开发风险评估、AI 内审、人工确认。触发场景：方案设计、step1-design、出方案、开发风险评估、step1-risk"
---
...
`step1-risk` / 开发风险评估 / 风险评估（仅重做或补齐风险产物时）
...
1. 设计技术方案，包含：
   ...
   - 数据模型变更
   - 模块影响范围
   - 方案级风险摘要（详细开发风险见下一步独立产物）
2. 按 `docs/harness/templates/技术方案文档模板.md` 结构编排方案
3. 写入 `docs/versions/<版本>/<需求>/技术方案.md`
4. API 或配置行为变更时同步 `docs/api/` 或 `docs/guide/`

### 第四步：开发风险评估（强制环节）

> 技术方案中的「风险与应对」不等于本环节。本环节评估**开发实现**风险，并独立落盘。

1. 按 `docs/harness/templates/开发风险评估模板.md` 编写评估
2. 至少覆盖类别：实现、集成、迁移、回归、性能、安全、排期（无则写「不适用」）
3. Critical / High 必须写清：触发条件、最坏结果、缓解措施、**可执行验证方式**
4. 写入 `docs/versions/<版本>/<需求>/开发风险评估.md`
5. **Critical 开放数必须为 0** 后才能进入人工确认 / Gate 1

仅触发 `step1-risk` 时：读取已有技术方案，补齐或修订本文件，不重写整份方案。

### 第五步：AI 内审

- 逐章节检查方案一致性
- 检查接口设计与现有架构一致性
- 对照 `开发风险评估.md`：Critical=0；High 均有应对
- 识别 Critical 问题并修复

### 第六步：人工确认

- 用户逐章节审核技术方案
- 用户审核开发风险评估（尤其 Critical/High）
- 用户确认方案与风险应对可行
...
- 更新 `workflow_state.md`：Step 1 状态 → ✅ 已完成，记录完成时间；产物清单勾选技术方案 + 开发风险评估
- 提示用户可执行 `step2-plan` 进入任务规划阶段（将自动检查 Gate 1，含风险评估落盘）。
