...
| `⚠️` 系统代理 MITM | `/api/v1/proxy/pac`, `/api/v1/proxy/ca.crt`, `/api/v1/proxy/status`, `/api/v1/proxy/setup/status`, `/api/v1/proxy/egress-key/*`, `/api/v1/proxy/domains/*`, `/api/v1/proxy/patterns/*` | 11 | 桌面/Team 本机出口；`setup/status` 含 lan/egress；`egress-key/ensure|bind` 热绑定出口 Key | 保留；产品化见 v0.2.5 |
...
| 分组 | 路径 | 端点数 | 说明 | 待确认事项 |
|------|------|--------|------|-----------|
| Agent 快速配置 | `/api/v1/agent/*` | 6 | Agent 工具配置生成 / 写入 / 恢复默认 | 便利功能，可否移至 WebUI 前端实现 |
