...
2. **对话记录能力不完整**：现有 `user_request_logs` 偏审计摘要，缺少统一的 conversation/session 模型与「用户输入 + 流水线输出」语义；也未按 minimal/personal/team 做存储后端分流。
...
| `dist/personal` | `personal`（**Profile 固定**） | 同 minimal：个人计量 + backend/model 细分 | **SQLite** |
| `dist/team` | `team` | 用户独立计量 + 计费/配额 | **PostgreSQL** |

> 说明：代码里 `edition` 枚举为 `minimal | personal | team`；`personal` 为发行包名，运行时对应 `personal`。  
> 配置真源：`config/profiles/personal/.env.example` 与 `docker-compose.yaml` 均固定 `CENTAG_EDITION=personal`。
...
| 版本 | 计量主体 | 细分 | 计费 |
|------|----------|------|------|
| minimal / personal | 单用户/本机（`user_id` 可为 0 或默认用户） | **必须**可按 `backend`、`model` 独立查询聚合 | 成本估算（pricing）即可，不做多用户扣费 |
...
4. **发行版**：`CENTAG_EDITION` / bootstrap 默认值与 dist 文档对齐；personal 文档明确推荐 `personal`。
...
- `personal` 与 `personal` 命名需在用户文档写清，避免只改 dist 名不改 edition。
