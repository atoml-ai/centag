...
> **デフォルトログイン：** ユーザー名 `admin` —— 初回起動の初期化ウィザードでパスワードを設定（プリセットなし）。初回起動前に `LLM_PROXY_ADMIN_PASSWORD` でプリセットも可能。
...
永続化データは `deploy/docker/data/` ディレクトリに保存（初回起動時に自動作成）。

<details>
<summary>ネイティブ Docker コマンド（代替）</summary>
