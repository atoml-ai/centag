...
echo "Building Centag Personal (full-feature; same plugins as team)..."
go mod tidy
go build -tags "$TAGS" -ldflags="-s -w" -o "$ROOT/bin/server/centag-personal" .
echo "✅ centag-personal built ($(ls -lh "$ROOT/bin/server/centag-personal" | awk '{print $5}'))"
