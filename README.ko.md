...
> **기본 로그인:** 사용자 이름 `admin` — 최초 시작 시 초기화 마법사에서 비밀번호 설정(사전 설정 없음). 첫 시작 전 `LLM_PROXY_ADMIN_PASSWORD`로 사전 설정도 가능.
...
cd centag
mkdir -p ~/.centag && cp config/centag.conf.example ~/.centag/centag.conf   # 필요 시 수정
